...
def storeJourneys(connection, journeyType, journeys):
    with connection.cursor() as cursor:
        ...
        for journey in journeys["outward"]:
            journeysConverted.append(journeyToArray(journeyType + "Out", journey))
        for journey in journeys["inward"]:
            journeysConverted.append(journeyToArray(journeyType + "In", journey))
        cursor.executemany(
...
def journeyToArray(journeyType, journey):
    ...
    for change in journey["changes"]:
        changes.append((change["startCode"], change["endCode"], change["operatorCode"], change["operatorName"], change["mode"]))
    journeyArray.append(changes)
    journeyArray.append(journey["changes"][0]["startCode"])
    journeyArray.append(journey["changes"][len(journey["changes"])-1]["endCode"])
    fares = []
    for fare in journey["fares"]:
...
if __name__ == "__main__":
    ...
    with psycopg.connect("dbname=db host=db user=admin password=admin") as connection:
        print("connected")
        # request = TrainJourneySearchRequest("SOC", "182", "09:15:00", StartType.ARRIVE, "17:30:00", StartType.DEPART, DayOfWeek.TUE, "YNG")
